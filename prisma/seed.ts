...
async function createKeylogs(devices) {
  ...
  const applicationContents = [
    ...
    "Microsoft Excel - Budget2023.xlsx",
    "Adobe Photoshop",
    "Visual Studio Code",
    "Slack",
    "Microsoft Teams",
  ...
  const browserContents = [
    ...
    "https://internal.company.com/reports",
    "https://internal.company.com/sensitive",
    ...
    for (let i = 0; i < numKeylogs; i++) {
      ...
      switch (type) {
        case "Application":
          content =
            applicationContents[
              Math.floor(Math.random() * applicationContents.length)
            ];
          break;
        case "Browser":
          content =
            browserContents[Math.floor(Math.random() * browserContents.length)];
          break;
        case "System":
          content =
            systemContents[Math.floor(Math.random() * systemContents.length)];
          break;
      }

      // Some keylogs are flagged
      const flagged = Math.random() < 0.2; // 20% chance of being flagged
